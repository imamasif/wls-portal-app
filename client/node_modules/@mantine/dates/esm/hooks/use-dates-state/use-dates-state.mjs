...
function useDatesState({ type, level, value, defaultValue, onChange, allowSingleDateInRange, allowDeselect, onMouseLeave }) {
	...
	const onDateChange = (date) => {
		...
		if (type === "multiple") {
			if (_value.some((selected) => dayjs(selected).isSame(date, level))) setValue(_value.filter((selected) => !dayjs(selected).isSame(date, level)));
			else setValue([..._value, date]);
			return;
		}
		if (_value && allowDeselect && dayjs(date).isSame(_value, level)) setValue(null);
		else setValue(date);
	};
	...
	const isFirstInRange = (date) => {
		...
		if (dayjs(date).isSame(_value[0], level)) return !(hoveredDate && dayjs(hoveredDate).isBefore(_value[0]));
		return false;
	};
	const isLastInRange = (date) => {
		if (_value[1]) return dayjs(date).isSame(_value[1], level);
