...
const require_use_collapse = require("./use-collapse.cjs");
...
function useHorizontalCollapse({ transitionDuration, transitionTimingFunction = "ease", onTransitionEnd, onTransitionStart, expanded, keepMounted }) {
	...
	const onTransitionEndEvent = (0, react.useEffectEvent)(() => onTransitionEnd?.());
	...
	const transitionRef = (0, react.useRef)(0);
	require_use_did_update.useDidUpdate(() => {
		...
		if (expanded) window.requestAnimationFrame(() => {
			...
			window.requestAnimationFrame(() => {
				...
				if (!require_use_collapse.isMeasured(width)) {
					setStyles({});
					setState("entered");
