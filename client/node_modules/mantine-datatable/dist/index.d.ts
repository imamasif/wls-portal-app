...
import { JSX, Ref, RefObject } from 'react';
import { PopoverProps, MantineTheme, MantineStyleProp, TableTrProps, MantineSize, TextProps, PaginationProps, MantineColor, MantineLoader, CollapseProps, CheckboxProps, StylesRecord, MantineShadow, ScrollAreaProps, TableProps } from '@mantine/core';
 ...
 */
declare const useDataTableColumns: <T>({ key, columns, getInitialValueInEffect, headerRef, scrollViewportRef, }: {
    /**
    ...
    readonly setColumnsOrder: (order: string[]) => void;
    readonly columnsOrder: string[];
    readonly resetColumnsOrder: () => void;
    readonly columnsToggle: DataTableColumnToggle[];
    readonly setColumnsToggle: (toggle: DataTableColumnToggle[]) => void;
    readonly resetColumnsToggle: () => void;
    readonly columnsPinning: DataTableColumnPinning[];
    readonly setColumnsPinning: (pinning: DataTableColumnPinning[] | ((prev: DataTableColumnPinning[]) => DataTableColumnPinning[])) => void;
    readonly resetColumnsPinning: () => void;
    readonly columnsWidth: {
        [x: string]: string | number;
    }[];
    readonly setColumnsWidth: (updates: {
        accessor: string;
        width: string | number;
    }[]) => void;
    readonly setColumnWidth: (accessor: string, width: string | number) => void;
    readonly setMultipleColumnWidths: (updates: {
        accessor: string;
        width: string | number;
    }[]) => void;
    readonly resetColumnsWidth: () => void;
    readonly hasResizableColumns: boolean;
    readonly allResizableWidthsInitial: boolean;
    readonly measureAndSetColumnWidths: () => void;
 ...
 */
declare function sanitizeStoredArray<T>(value: unknown, fallback: T[], isValidItem?: (item: unknown) => boolean): T[];
/**
 * Predicate matching the `{ accessor: string, ... }` shape persisted for column
 * toggle and pinning state.
 */
declare function hasStringAccessor(item: unknown): boolean;
/**
 ...
 */
declare function getMaxGroupDepth<T>(groups: readonly DataTableColumnGroup<T>[]): number;
/**
 * Flattens nested column groups to extract all columns at the deepest level
 */
declare function flattenColumns<T>(groups: DataTableColumnGroup<T>[]): DataTableColumn<T>[];
/**
 * Calculates the column span for a group based on visible columns
 */
declare function calculateColSpan<T>(group: DataTableColumnGroup<T>, visibles?: (boolean | undefined)[]): number;
/**
 * Gets all groups at a specific depth level
 */
declare function getGroupsAtDepth<T>(groups: readonly DataTableColumnGroup<T>[], targetDepth: number, currentDepth?: number): DataTableColumnGroup<T>[];
/**
 * Checks if a group needs a right border based on its position and context
 */
declare function needsRightBorder(isLastGroup: boolean, hasMoreColumnsAfter: boolean, withColumnBorders: boolean): boolean;

export { DataTable, type DataTableCellClickHandler, type DataTableColumn, type DataTableColumnGroup, type DataTableColumnPinning, type DataTableColumnTextAlign, type DataTableColumnToggle, type DataTableDefaultColumnProps, DataTableDraggableRow, type DataTableDraggableRowProps, type DataTableEmptyStateProps, type DataTableOuterBorderProps, type DataTablePaginationProps, type DataTablePaginationRenderContext, type DataTableProps, type DataTableRowClickHandler, type DataTableRowExpansionCollapseProps, type DataTableRowExpansionProps, type DataTableSelectionProps, type DataTableSelectionTrigger, type DataTableSortProps, type DataTableSortStatus, type DataTableVerticalAlign, type WithOptionalProperty, type WithRequiredProperty, calculateColSpan, differenceBy, flattenColumns, getGroupsAtDepth, getMaxGroupDepth, getRecordId, getValueAtPath, hasStringAccessor, humanize, needsRightBorder, reorderRecords, sanitizeStoredArray, swapRecords, uniqBy, useDataTableColumns };
