...
function formatDriveEmbedUrl(url) {
  ...
  if (cleanUrl.includes("drive.google.com")) {
    ...
    if (!fileIdMatch || !fileIdMatch[1]) {
      return {
        embedUrl: "",
        error:
          "Invalid Google Drive URL format. Ensure it contains a valid File ID.",
      };
    }
  ...
  return {
    embedUrl: "",
    error: "Unrecognized URL or invalid video stream link.",
  };
}
...
function extractNormalizedScores(rawAssessment, currentAdminId) {
  ...
  if (
    Array.isArray(rawAssessment.evaluations) &&
    rawAssessment.evaluations.length > 0
  ) {
    ...
    const targetEval =
      adminEval ||
      rawAssessment.evaluations[rawAssessment.evaluations.length - 1];
...
export function WlsAssessmentPanel({
  ...
  const loadPanelData = async (targetSessionId = null) => {
    try {
      ...
      const { sessions, allUsers, rulesData } =
        await fetchAssessmentPanelData(currentAdminId);

      if (Array.isArray(sessions)) {
        setAllSessions(sessions);
      }

      const rawRules = Array.isArray(rulesData)
        ? rulesData
        : rulesData?.rules || rulesData?.data || [];

      if (Array.isArray(rawRules) && rawRules.length > 0) {
        const activeRules = rawRules
          .filter((r) => r.isActive !== false)
          .map((r) => ({
            key: r.key || r.id || r._id,
            title: r.criterion || r.title || r.name || r.key,
        ...
        setCriteriaList([
          {
            key: "presentation",
            title:
              "1. Presentation - camera position, Light, Picture and Sound Quality - Video Size",
          },
      ...
      if (activeSession) {
        ...
        setSessionInfo({
          ...
          date:
            activeSession.sessionDateTimeToronto ||
      ...
      if (resolvedSessionId) {
        try {
          const saved = localStorage.getItem(
            `wls_draft_scores_${resolvedSessionId}_${currentAdminId}`,
          );
          if (saved) {
      ...
      if (activeSession?.groupAssignments) {
        Object.values(activeSession.groupAssignments).forEach((group) => {
          const isAssignedAdmin =
            !currentAdminId || group.adminIds?.includes(currentAdminId);
      ...
      for (const studentId of assignedStudentIds) {
        ...
        if (resolvedSessionId) {
          try {
            const res = await fetch(
              `/api/assessments/session/${resolvedSessionId}/user/${studentId}`,
            );
            if (res.ok) {
            ...
            console.error(
              `Could not fetch assessment for student ${studentId}:`,
              e,
            );
          }
        }

        const sessionSubRecord =
          sessionSubmissions instanceof Map
            ? sessionSubmissions.get(studentId)
            : sessionSubmissions[studentId];

        const studentUrl =
          assessment.submissionUrl ||
          sessionSubRecord?.submissionUrl ||
        ...
        if (
          !computedMap[studentId] ||
          Object.keys(computedMap[studentId]).length === 0
        ) {
          computedMap[studentId] = extractNormalizedScores(
            assessment,
            currentAdminId,
          );
        }
      ...
      if (memberList.length > 0) {
        ...
        const matchingMember = prevSelectedId
          ? memberList.find((m) => m.id === prevSelectedId)
          : null;
        ...
        setVideoUrl(
          targetUser.adminSubmissionUrl || targetUser.submissionUrl || "",
        );
  ...
  const handleRefreshCurrentStudent = async () => {
    ...
    try {
      const res = await fetch(
        `/api/assessments/session/${selectedSessionId}/user/${selectedUser.id}`,
      );
      if (res.ok) {
        const assessment = await res.json();
        const updatedMessages = assessment.messages || [];
        const updatedEvalsArray = assessment.evaluations || [];
        const normalizedScores = extractNormalizedScores(
          assessment,
          currentAdminId,
        );
        ...
        setUserScoresMap((prev) => ({
          ...prev,
          [selectedUser.id]: normalizedScores,
        }));
  ...
  const handleUserSelect = (user) => {
    ...
    if (!userScoresMap[user.id]) {
      const evaluationsList = Array.isArray(user.evaluations)
        ? user.evaluations
        : [];
      const existingEval = evaluationsList.find(
        (e) => String(e?.evaluatorId) === String(currentAdminId),
      );
      const initialScores = existingEval
        ? extractNormalizedScores(
  ...
  const handleScoreChange = (criterionKey, score) => {
    ...
    const existingEvals = Array.isArray(selectedUser.evaluations)
      ...
      : [];
    const filteredEvals = existingEvals.filter(
      (e) => String(e?.evaluatorId) !== String(validEvaluatorId),
    );
    ...
    try {
      ...
      if (!targetAssessmentId) {
        ...
        const activeStreamUrl =
          adminVideoUrl || selectedUser.submissionUrl || "";
    ...
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm" c="gray.7" mb="md">
          Are you sure you want to save grades for{" "}
          <strong>{selectedUser.name}</strong>? Select{" "}
          <strong>Save as Completed</strong> to mark status as fully completed,
          or <strong>Partial Save</strong> to save progress and resume later.
        </Text>
      ),
  ...
  const activeScores =
    selectedUser && userScoresMap[selectedUser.id]
      ? userScoresMap[selectedUser.id]
      : {};
  const activeVideoToRender = adminVideoUrl || videoUrl;
  const { embedUrl, error: urlCheckError } =
    formatDriveEmbedUrl(activeVideoToRender);
